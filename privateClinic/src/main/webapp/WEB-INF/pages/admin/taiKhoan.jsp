...
<nav class="navbar navbar-expand-sm navbar-dark " style="background-color: #e9ecef">
    <div class="container-fluid">
        <a class="navbar-brand" href="javascript:void(0)" style="color: black; font-weight: 500">Danh sách tài khoản</a>
        ...
        <div class="collapse navbar-collapse" id="mynavbar">
            ...
            <c:url value="/admin/taikhoan" var="action"></c:url>
            <form class="d-flex" action="${action}" style="margin-top: 10px">
                <input class="form-control me-2" type="text" name="kw" placeholder="Tìm tên tài khoản...">
...
<section class="container" style="margin-top: 15px">
    <table class="table table-hover" style="border-collapse: collapse;border: 1px solid lightgray">
        ...
        <tr style="margin-left: 10px">
            ...
            <th style="width: 200px; text-align: center">Tài Khoản</th>
            <th style="width: 100px; text-align: center">Quyền</th>
            <th style="width: 100px; text-align: center">Trạng Thái</th>
            <th style="width: 100px; text-align: center">Mã NV</th>
            <th style="width: 110px; text-align: center"></th>
        </tr>
        </thead>
        <tbody>
        <c:forEach items="${taikhoan}" var="b">
            <tr>
                <td>
                    <img src="${b.avatar}" alt="${b.name}" style="width: 60px; height: 60px">
                </td>
                <td style="text-align: center">${b.id}</td>
                <td style="text-align: center">${b.name}</td>
                <td style="text-align: center">${b.username}</td>
                <td style="text-align: center">${b.userRole}</td>
                <td style="text-align: center">${b.isActive}</td>
                <td style="text-align: center">${b.maNv}</td>
                <td>
                    <c:url value="/api/taikhoan/${b.id}" var="apiDel"/>
                    <a href="<c:url value="/admin/taikhoan/${b.id}" />" class="btn " style="background-color: lightgray">Sửa</a>
                    <button class="btn btn-danger" onclick="delPro('${apiDel}', ${b.id})">Xóa</button>
                </td>
            </tr>
        </c:forEach>
